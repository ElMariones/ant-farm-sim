...
**Implemented — food you can see.** Forage sites are a fallen berry ringed with seed husks or a dead beetle, shrinking as the colony carries them away and gone when it has. A site the colony has not found yet is drawn only as a faint outline, so watching a scout stumble onto it is the moment it becomes real; the site the colony is being called to pulses while the alert lasts. Rooms read as a soft warmth on the chamber floor, warmer for brood than for grain, so brood and grain sit inside a place rather than on bare rock. Stored food is drawn where it actually is: every heap in a store chamber grows up from the chamber floor as the pile fills, in the nutrient's own colour, and collapses to a tinted cell below zoom 2.2. Brood held by a nurse rides on her interpolated pose rather than hopping cell to cell.
