...
void validate_world(const sim::WorldSnapshot& w) {
  ...
  for(const auto& actor:w.actors){add(actor.identity.id); if(actor.identity.id<=previous) throw std::invalid_argument("actors are not in stable id order"); previous=actor.identity.id; if(!passable(actor.position.cell())||actor.cargo.amount<0||actor.worker!=(actor.ant.kind==sim::AntKind::Worker)) throw std::invalid_argument("invalid actor state"); if(!actor.worker){if(actor.ant.kind==sim::AntKind::Queen){if(queen) throw std::invalid_argument("multiple queens");queen=true;}continue;} if(actor.movement.next_cell>actor.movement.path.size()||actor.movement.path.size()>cells||actor.movement.speed_residual<0||actor.movement.speed_residual>=sim::kTicksPerSecond) throw std::invalid_argument("invalid movement state"); for(const auto p:actor.movement.path) if(!passable(p)) throw std::invalid_argument("path crosses an impassable cell"); if(actor.forager.source_index<-1||actor.forager.source_index>1||actor.forager.reserved_amount<0) throw std::invalid_argument("invalid reservation"); if(actor.forager.source_index>=0) reservations[static_cast<std::size_t>(actor.forager.source_index)]+=actor.forager.reserved_amount;}
